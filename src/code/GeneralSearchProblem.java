    ...
    public static boolean isRedundantState(Node n1, ArrayList<Node> expanded) {
        ...
        for (int i = 0; i < expanded.size(); i++) {
            ...
            if (n1.state.i == pre_node.state.i && n1.state.j == pre_node.state.j  && n1.operator.equals("retrieve") && pre_node.operator.equals("retrieve") ){
                System.out.println("retrive redundant"+n1.state.i+" "+n1.state.j);
                            return true;}
    ...
    public static String BFS(GridCell[][] grid, Node cg, int capacity, int totalPassengers) {
        ...
        while (!q.isEmpty()) {
            ...
            if (cg.goalTest(capacity)) {
                pass_goal = cg;
                String result = printPath(pass_goal) + ";" + totalPassengers + ";" +retrieved_blackboxes+";"+ count_nodes_expanded; // number of deaths, number of retrieved boxes?
            ...
            if (cg_i < grid.length - 1) {
                q.add(new Node(cg_i + 1, cg_j,
                        ...
                        cg.state.remaining_capacity, cg.state.rescued_passengers, "down", cg, cg.state.observers));
            }
            if (cg_i > 0) {
                q.add(new Node(cg_i - 1, cg_j,
                        cg.state.remaining_passengers, cg.state.remaining_blackboxes,
                        cg.state.remaining_capacity, cg.state.rescued_passengers, "up", cg, cg.state.observers));
            }
            if (cg_j < grid[0].length - 1) {
                q.add(new Node(cg_i, cg_j + 1,
                        cg.state.remaining_passengers, cg.state.remaining_blackboxes,
                        cg.state.remaining_capacity, cg.state.rescued_passengers, "right", cg, cg.state.observers));
            }
            if (cg_j > 0) {
                q.add(new Node(cg_i, cg_j - 1,
                        cg.state.remaining_passengers, cg.state.remaining_blackboxes,
                        cg.state.remaining_capacity, cg.state.rescued_passengers, "left", cg, cg.state.observers));
            }
...
//                        ship.passengers -= passengers_carried;
                        q.add(new Node(cg_i, cg_j,
                                cg.state.remaining_passengers, cg.state.remaining_blackboxes,
                                cg.state.remaining_capacity , cg.state.rescued_passengers, "pickup", cg, cg.state.observers));

                    } else if (ship.wrecked && ship.black_box <100 && !ship.done) {

                        q.add(new Node(cg_i, cg_j,
                                cg.state.remaining_passengers, cg.state.remaining_blackboxes,
                                cg.state.remaining_capacity, cg.state.rescued_passengers, "retrieve", cg, cg.state.observers));
                    }
                ...
                if (grid[cg_i][cg_j] instanceof Station && cg.state.remaining_capacity < capacity) {
                    ...
                    q.add(new Node(cg_i, cg_j,
                            ...
                            capacity, cg.state.rescued_passengers + passengers_dropped_off, "drop", cg, cg.state.observers));
                }
            }
        }
